...
const websocketUrl = "https://hrk118cb-8080.use2.devtunnels.ms/notify-channels"
const topic = "/group/";
var client = null;

let currentProject = null, isSave = true, actualPosition = 0
	, daysToForward = 0, daysToBackward = 0, currentSurface,
	temp, currentGroup, calendarWidth = 480, userPermiss = null;
...
function sendMessage(gid, message) {
...
/**
* Creates a new list projects
*
* @param token - the user authentication JWT token
*
*
*/
...
staticLinks.forEach((s) => {
	s.addEventListener("click", (e) => {
		location.href = "https://hrk118cb-8080.use2.devtunnels.ms/" + e.currentTarget.dataset.id;
	})
})

linkAuth.addEventListener("click", (e) => {
	if (e.currentTarget.dataset.id == "login") {
		location.href = "https://hrk118cb-8080.use2.devtunnels.ms/auth?from=projectmanager";
	} else {
		logout();
	}
});
